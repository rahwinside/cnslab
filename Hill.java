...
public class Hill
{
    ...
    public void divide(String temp, int s)
    {
        while (temp.length() > s)
        {
            String subj = temp.substring(0, s);
    ...
    {
        linetom(line);
        line_multiplykey(line.length());
        result(line.length());
    }
 
    public void keytom(String key, int len)
    {
        key_m = new int[len][len];
        int c = 0;
        for (int i = 0; i < len; i++)
        {
            for (int j = 0; j < len; j++)
            {
                key_m[i][j] = ((int) key.charAt(c)) - 97;
                c++;
            }
        }
    }
 
    public void linetom(String line)
    {
        line_m = new int[line.length()];
        for (int i = 0; i < line.length(); i++)
        {
            line_m[i] = ((int) line.charAt(i)) - 97;
        }
    }
 
    public void line_multiplykey(int len)
    {
        result_m = new int[len];
        for (int i = 0; i < len; i++)
        {
            for (int j = 0; j < len; j++)
            {
                result_m[i] += key_m[i][j] * line_m[j];
            }
            result_m[i] %= 26;
        }
    }
